            ...
            if (bag.weapon is Gun gun)
            {
                if (Input.GetKey(KeyCode.Mouse0))
                {
                    ...
                    if (attackTimer < 0)
                    {
                        ...
                        GameObject bullet = Instantiate(gun.bullet, PlayerManager.instance.playerTransform.position + new Vector3(gunVector.x * .25f, gunVector.y * .25f, -.25f), Quaternion.identity, ItemManager.instance.itemTransform);//z轴高度生成后可能需要调整碰撞体的判断高度
