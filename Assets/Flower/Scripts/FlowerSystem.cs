...
namespace Flower{
    ...
    public class FlowerSystem : MonoBehaviour
    {
        ...
        private void Awake() {
            ...
            RegisterEffect("uiImageFadeIn", EffectUIImageAlphaFadeIn);
            RegisterEffect("uiImageFadeOut", EffectUIImageAlphaFadeOut);
       ...
       public void SetupDialog(string resourcePath="DefaultDialogPrefab", bool applyDefaultTextUpdateEvent=true, bool applyDefaultTextSplitting=false){
            try{
                ...
                if(applyDefaultTextUpdateEvent){
                    try{
                        ...
                        var textRect = dialog.transform.Find("DialogPanel/DialogText").GetComponent<RectTransform>();
                        text.text = "";
                        if(applyDefaultTextSplitting){
                            text.horizontalOverflow = HorizontalWrapMode.Overflow;
                        }
                        _defaultTextUpdateHandler = (object sender, TextUpdateEventArgs args)=>{
                            text.text = args.text;
                            if(applyDefaultTextSplitting){
                                if(text.preferredWidth > textRect.rect.width){
                                    msgText.Insert(msgText.Length-1,"\n");
                                }
                            }
                        };
        ...
        {  
            ...
            try{
                ...
                audioSource = sceneObj.GetComponent<AudioSource>();
        ...
        private IEnumerator CmdFunc_ui_image_Task(List<string> _params)
        {
            string key;
            Sprite sp;
            int x;
            int y;
            string uiStageKey="default";
            string effectName="";
            try{
                key = _params[0].ToString();
                sp = LoadResource<Sprite>(_params[1].ToString());
            ...
            GameObject sceneObj = GetSceneObject(key, false);
            try{
                GameObject uiStage = GetSceneObject("_UIStage_"+uiStageKey) ?? throw new Exception($"UI Stage - ${uiStageKey} not exists.");
                
                var imagePrefab = LoadResource<GameObject>(GetPrefabPath("ui_image"));
                if(!sceneObj){
                    sceneObj = CreateAsSceneObject(key, imagePrefab, new Vector3(ConvertPixelToUnit(x), ConvertPixelToUnit(y), 0));
                }
        ...
        private void EffectUIImageAlphaFadeIn(string key, List<string> _params){
            try{
                float endTime;
                try{
                    endTime = float.Parse(_params[0])/1000;
                ...
                Image img = GetSceneObject(key).GetComponent<Image>();
                var startColor = img.color;
                startColor.a = 0;
                var endColor = img.color;
                StartCoroutine( ChangeUIImageColorTask($"fadeIn-{key}", img,  startColor, endColor, endTime));
            }catch(Exception){
                Log($"Effect - effectUIImageAlphaFadeIn @ [{key}] failed.",LogType.Error);
            }
        }
        private void EffectUIImageAlphaFadeOut(string key, List<string> _params){
            try{
                float endTime;
                try{
                    endTime = float.Parse(_params[0])/1000;
                ...
                Log($"Effect - effectUIImageAlphaFadeOut @ [{key}] failed.",LogType.Error);
            }
        }
        #endregion
